...
const splitIntoWords = (input) => {
  return new Promise(async callback => {
    ...
    const text = input.toLowerCase().replace(/[-·'"`!?()]/g, ' ').replace(/\u00AD/g /* Soft hyphens */ , '').trim()
    const split = text.split(wordRegex)
    let results = []

    await split.filter(i => i.length < 50).forEachAsync(async (word, i) => {
...
const lookup = (input, callback) => {
  query('SELECT * FROM pronunciation WHERE word = ?', [input], (error, results, fields) => {
  ...
  */
  ...
  for (var start = 0; start <= input.length; start++) {
    for (var end = 1; start + end <= input.length; end++) {
      ...
      if (word.length > 1 || (word.length == 1 && /[aeioóú]/.test(word) /* Vowels can be single */ )) {
  ...
  query(`SELECT * FROM pronunciation WHERE (${select}) AND guessed IS NULL ORDER BY LENGTH(word) DESC`, (error, results, fields) => {
    ...
    */
    const returns = FindPossibleWaysToFit(input, list).map(possibility => {
        const split = possibility.text.split('-')
        const parts = split.length
        const average_part_length = split.map(part => part.length).reduce((p, c) => { return p + c }) / split.length
        ...
        const score = average_part_length / parts / (1 + einstæðar + tvístæðar * 0.4 + þrístæðar * 0.1)
        return {
      ...
      .reduce((p, c) => { return p.score > c.score ? p : c }, {})
...
const save_guess = (word, pronunciation, callback) => {
  query(`INSERT INTO pronunciation (word,pronunciation,guessed) VALUES ('${word}', "${pronunciation}", TRUE)`, (error, results, fields) => {
    if (error) throw error;
    callback()
  })
}
