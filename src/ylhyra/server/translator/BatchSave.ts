...
const run = async (apcontinue = "") => {
  const page_list = (
    await axios.get(
      `https://ylhyra.is/api.php?action=query&format=json&list=allpages&apnamespace=3000&aplimit=20&apcontinue=${apcontinue}`
    )
  ).data;
  ...
  await page_list.query.allpages.forEachAsync(async (data_info) => {
    ...
    await new Promise(async (resolve) => {
      ...
      const info = (
        await axios.get(
          `https://ylhyra.is/api.php?action=query&format=json&titles=${title}&prop=info`
        )
      ).data;
      let pageid, ns;
      ...
      const data = (
        await axios.get(
          `https://ylhyra.is/index.php?title=${datatitle}&action=raw&ctype=text/json&random=${Math.random()}`
        )
      ).data;
