...
export const updateDefinition = ({ definition, selected }) => (dispatch, getState) => {
  dispatch({
...
export const updateDefinitionValue = ({ name, value }) => (dispatch, getState) => {
  const selected = getState().editor.selected
  dispatch({
...
export const wordsHash = ids => (
  hash(ids.join('')).toString(36)
)
