...
(0, database_1.default)(`SET sql_mode = ''`, () => { });
setTimeout(() => {
    (0, database_1.default)(`SET sql_mode = ''`, () => { });
}, 30 * 1000);
const mainapp = (0, express_1.default)();
mainapp.use(paths_1.processed_image_url, (0, caching_1.staticCached)(paths_backend_1.image_output_folder));
mainapp.use(paths_1.unprocessed_image_url, (0, caching_1.staticCached)(paths_backend_1.ylhyra_content_files));
mainapp.use("/", (0, caching_1.staticCached)(paths_backend_1.build_folder));
/*
  Private APIs
*/
...
mainapp.use("/api", require("server/vocabulary/migration_session_log_2021_10").default);
...
const inflections_app = (0, express_1.default)();
inflections_app.use(cors({ origin: "*" }));
inflections_app.set("json spaces", 2);
inflections_app.use("/inflection_styles.css", (0, caching_1.staticCached)(path_1.default.join(process.env.PWD, "/build/inflection_styles.css")));
inflections_app.use("/", require("inflection/server/server-with-database/route_loader").default);
app.use((req, res, next) => {
    if (/inflections\./.exec(req.headers.host) ||
...
process.on("uncaughtException", (err) => {
    if (isDev_1.isDev) {
        (0, child_process_1.exec)(`terminal-notifier -group 'ylhyra' -title '⚠️' -message 'Server has crashed'`);
    }
    console.error(err);
});
