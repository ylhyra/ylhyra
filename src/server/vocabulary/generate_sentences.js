...
const spaces = /[ ,."?!;:\-–“„]/g;
const run = () => {
    fs.readFile(__basedir + `/build/vocabulary_database.json`, "utf8", (err, data) => {
        const vocabulary = JSON.parse(data);
        fs.readFile(path.resolve(__basedir + "./../Desktop/Ylhýruskjöl/Línur_úr_Facebook_samræðum.txt"), "utf8", (err, data) => __awaiter(void 0, void 0, void 0, function* () {
            let sortKeys = yield (0, sortKeys_1.default)(true);
            Object.keys(vocabulary.cards).forEach((card_id) => {
                /* Hei þetta split() virkar ekki... */
                [
                    (0, format_1.getPlaintextFromFormatted)(vocabulary.cards[card_id].is_formatted),
                    "lemmas",
                    "alternative_id",
                ].forEach((c) => {
                    c.split(/[;,]+/g).forEach((s) => {
                        const j = (0, functions_1.getHash)(s.replace("%", ""), {
                            skip_hash: true,
                        });
                        if (!(j in sortKeys)) {
            ...
            Object.keys(sortKeys).forEach((sentence) => {
                sentence
                    ...
                    known_words[word] = Math.min(known_words[word] || 2000, sort_key);
                    if (sort_key < 1000) {
            ...
            data
                ...
                full_line
                    ...
                    if (!line)
                        return;
                    const lower = (0, functions_1.getHash)(line, { skip_hash: true }) || "";
                    let max_sortkey = 0;
                    let fail = false;
                    if (lower in sortKeys)
                        return;
                    // console.log(line);
                    // if (i > 5) process.exit();
            ...
            fs.writeFileSync(__basedir + "/../Desktop/Ylhýruskjöl/Missing_words.txt", Object.keys(word_frequency)
            ...
            fs.writeFileSync(__basedir +
                "/../Desktop/Ylhýruskjöl/Setningar úr orðum sem eru í námskeiðinu.txt", matches
