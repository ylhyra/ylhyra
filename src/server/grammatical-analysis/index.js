...
export default (tokenized) => {
  return new Promise(async (resolve, reject) => {
    ...
    const output =
      merge_tokenization_and_analysis_to_create_suggestions(output_sentences);
    resolve(output);
  });
};
...
const merge_tokenization_and_analysis_to_create_suggestions = (sentences) => {
  ...
  Object.keys(sentences).forEach((sentence_id) => {
    ...
    let w_index = 0;
    const w_text_array = tokenization.map((word) =>
      (word.text || word).trim().replace(/ /g, "")
    );
    const a_text_array = analysis.map((x) =>
      (x.text || x).trim().replace(/ /g, "")
    ); // Tries to match ignoring spaces
