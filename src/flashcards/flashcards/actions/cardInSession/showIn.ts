 ...
 * Only a single option (interval, minInterval, or
 * cannotBeShownUntilInterval) is ever used at a time.
...
export function showIn(
  this: CardInSession,
  {
    ...
    /**
     * Set the minimum queue position of a card, i.e. can
     * move cards back but can never move cards forward.
     * A hard requirement (cannotBeShownUntil) will also be set automatically.
     */
    minInterval?: IntervalRelativeToCurrentCardBeingAtZero;
    /**
     * Hard requirement for when a card can be shown.
     * Rarely used since the above options automatically get
     * a cannotBeShownUntil set. Mainly used to set a low
     * limit (e.g. 2) for a distantly related card but also
     * to force cards that depend on a bad on come later.
     */
  ...
  /**
   * Automatically set cannotBeShownUntil if
   * either interval or minInterval is set.
   */
