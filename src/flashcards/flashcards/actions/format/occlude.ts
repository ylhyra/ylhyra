...
/**
 * Occlusion is used to give the user hints. This is especially useful for
 * disambiguation purposes (meaning the user does not get confused regarding
 * which synonym he should be guessing).
 *
 * There are three ways to mark a section as being occluded:
 *
 * - Between asterisks: "b_la bla_" becomes "b[la bla]".
 * - After a single asterisk. This is used to occlude the entire remainder of the
 *   sentence. "b*la bla" becomes "b[la bla]".
 * - After a single percentage mark. This is used to occlude only the remainder of
 *   the current word. "b%la bla" becomes "b[la] bla".
 */
export const DocumentationRegardingOcclusion = "";

export function formatOcclusion(input: string): Html {
  return (
    input
      /**
       * Occlusion, {@see DocumentationRegardingOcclusion} Here, items between
       * aterisks are occluded and items after a single asterisk are occluded.
       */
      .replace(
        /( )?\*([^*;$!.,<>"=]+)\*?( )?/g,
        (
