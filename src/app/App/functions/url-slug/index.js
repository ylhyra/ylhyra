...
const REVERT_REGEXP =
  {}; /* RegExp intances are based on the separator and then cached */
...
function UrlSlug(separator, transform) {
  ...
  this.options = this.prepare(separator, transform);
...
UrlSlug.prototype.transformers = {
  ...
  titlecase: function (string) {
    return string
      .toLowerCase()
      .replace(TITLE_CASE_REGEXP, function (character) {
...
UrlSlug.prototype.prepare = function (separator, transform) {
  if (null == separator) {
    ...
    throw new Error(
      'Invalid separator, must be a string: "' + separator + '".'
    );
  } else if (INVALID_SEPARATOR_REGEXP.test(separator)) {
    throw new Error(
      'Invalid separator, has invalid characters: "' + separator + '".'
    );
  }
  ...
  if (null == transform) {
    ...
    throw new Error(
      'Invalid transform, must be a builtin transform or a function: "' +
        transform +
        '".'
    );
  }
...
UrlSlug.prototype.convert = function (string, separator, transform) {
  ...
  string = unidecode(string)
    .replace("[?]", "")
    .replace(CONVERT_REGEXP, "$1 $2")
    .trim();

  /* Pass string through transform function */

  if (this.options.transform) {
    string = this.options.transform(string);
  }

  /* Replace spaces with separator and return */

  return string.replace(CONVERT_SEPARATOR_REGEXP, this.options.separator);
};
...
UrlSlug.prototype.revert = function (slug, separator, transform) {
  ...
  if ("" === this.options.separator) {
    ...
    /* If separator argument was set as string, don't check this.options.separator,
        it can return the default separator (this.separator) */
    REVERT_REGEXP[separator] =
      REVERT_REGEXP[separator] ||
      new RegExp("\\" + separator.split("").join("\\"), "g");
  ...
  return transform && this.options.transform
    ? this.options.transform(slug)
    : slug;
};
