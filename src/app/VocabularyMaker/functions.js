...
export const clean_string = (i) => {
  return getRawTextFromVocabularyEntry(i);
  // return i
  //   .replace(/;;/g, "MAJOR_SEPERATOR")
...
export const parse_vocabulary_file = ({ rows, sound }) => {
  ...
  rows.forEach((row) => {
    ...
    raw_icelandic_strings.forEach((t) => {
    ...
    if (row.direction !== "<-") {
      raw_icelandic_strings.forEach((i, index) => {
        to_add.push({
          is: i,
    ...
    if (row.direction !== "->") {
      to_add.push({
        is: getRawTextFromVocabularyEntry(row.icelandic),
