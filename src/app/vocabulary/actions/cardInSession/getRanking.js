...
function getRanking() {
    const card = this;
    const id = this.getId();
    const from = (0, card_data_1.getFrom)(id);
    let q = this.getQueuePosition();
    // New terms are not relevant unless there are no overdue cards
    if (!(0, card_data_1.getTermIds)(id).some((term_id) => term_id in this.session.lastSeenTerms)) {
        q += 1000;
    }
    // Seen cards
    else {
    ...
    if (this.history[0] === constants_1.BAD &&
        q === 0 &&
    ...
    if (this.session.cardTypeLog[0] === from) {
        ...
        if (this.session.cardTypeLog[1] === from) {
            /* Two in a row */
            if (this.hasBeenSeenInSession() || !(0, card_schedule_1.isNewCard)(id)) {
                q += 5;
            }
            /* Three new cards in a row */
            if (this.session.cardTypeLog[2] === from &&
                // Only if a user says "Good" to all three previous
                !this.session.ratingHistory.slice(0, 3).some((i) => i === constants_1.BAD) &&
    ...
    if (!(0, card_data_1.getData)(id, "isSentence")) {
        ...
        // Delay words if no sentence has been seen for a while
        else if (this.session.ratingHistory.length >= 3 &&
            ...
            if (from === "en" || (0, card_schedule_1.isNewCard)(id)) {
                q += 20;
            }
        }
    }
