...
import { wordTooltip as Tooltip } from 'Editor/Translator/Views/Document/Tooltip'
...
const Element = styled.span `
  ...
  & > span {
    ...
    ${/* Hover */''}
    ${props => props.hover && !props.selected && `
      background: #fec96d;
      border-color: #eb9f5f;
    `}
  }
...
class Word extends React.Component {
  ...
  shouldComponentUpdate = (nextProps, nextState) => {
    ...
    if (definitions[words[id]] !== nextProps.translation.definitions[nextProps.translation.words[id]]) {
      return true
    }
  ...
  render() {
    ...
    if (definition && !inline_translation) {
      inline_translation = (definition.inline_translation || definition.meaning) || null
      if (inline_translation && definition.contains && definition.contains[0] !== id) {
        inline_translation = '←'
      }
    }
