...
import clean from 'User/Render/Elements/Vocabulary/functions/clean'
import { ParseHTMLtoObject, ParseHTMLtoArray, getTextFromReactElement as getText } from 'User/Render/Elements/parse'
...
class Element extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      answers: [null, null, null, null, null, null, null, null, null, null, null, null, null, null, null],
    })
  }
  ...
  render() {
      ...
      return (
        ...
        <div className="drag-drop-prompt">
          <div className="small gray sans-serif">Drag the words into the correct order.</div>
            ...
            <div>
              {options && options.map((option, index) => {
                ...
                if(answers&&answers.includes(text)) {
                  ...
                  return (
                    <Draggable key={index} dropped={(index)=>this.dropped(index,text)} className="" /*onClick={()=>this.click(text)}*/>
                      {option}
                    </Draggable>
                  )
                }
    ...
    if (input === null) {
      ...
      if (name === 'drag') {
        ...
        if (answers && answers[currentIndex]) {
          ...
          return <Droppable key={key} currentIndex={currentIndex}><span/></Droppable>
        }
      } else {
        return {
          ...input,
...
class Draggable extends Component {
  ...
  getTarget = (e) => {
    return document.elementFromPoint(e.srcEvent.clientX, e.srcEvent.clientY).closest('.ghost-target')
  }
...
class Droppable extends Component {
  render() {
    return (
      <div className="droppable drag-drop-target" data-current-index={this.props.currentIndex}>
