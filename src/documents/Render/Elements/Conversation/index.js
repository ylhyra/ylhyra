...
class Conversation extends React.Component {
  ...
  componentDidUpdate = (prevProps, prevState, snapshot) => {
    ...
    if (prevState.howManyToShow !== this.state.howManyToShow) {
      const last = this.state.conversation.slice(
        0,
  ...
  checkIfWeShouldGoToTheNext = () => {
    ...
    const last =
      this.state.conversation.slice(0, this.state.howManyToShow).last || {};
    if (
      last.type === "message" &&
      this.state.howManyToShow < this.state.conversation.length
    ) {
      this.next();
    }
  };
  ...
  render() {
    const { conversation, isInteractive, done, answer, card, howManyToShow } =
      this.state;
    // console.log({card, answer})
    return (
      <div className={`conversation ${isInteractive ? "interactive" : ""}`}>
            ...
            {done && (
              <div>
                <div className="button-container center">
                  <div className="button blue" onClick={this.reset}>
                    Repeat
                  </div>{" "}
                </div>
              </div>
