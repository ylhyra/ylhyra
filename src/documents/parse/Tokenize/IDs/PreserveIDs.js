...
const Preserve = (first, second) => {
    ...
    const PreservedIDs = Object.assign(Object.assign({}, DiffAndPreserveIDs(SentencesArray(first), SentencesArray(second))), DiffAndPreserveIDs(WordsArray(first), WordsArray(second)));
    /* Return with preserved IDs */
    return second.map((paragraph) => (Object.assign(Object.assign({}, paragraph), { sentences: paragraph.sentences.map((sentence) => (Object.assign(Object.assign({}, sentence), { id: PreservedIDs[sentence.id] || sentence.id, words: sentence.words.map((word) => {
                if (!word.id)
                    return word;
                return Object.assign(Object.assign({}, word), { id: PreservedIDs[word.id] || word.id });
            }) }))) })));
};
...
const DiffAndPreserveIDs = (first, second) => {
    ...
    let first_index = 0;
    let second_index = 0;
    const diff = (0, diff_1.diffArrays)(first.map((i) => i.text), second.map((i) => i.text));
    ...
    diff.forEach((part, part_index) => {
        if (diff[part_index + 1] &&
            ...
            removed.value.forEach((removed_value, removed_index) => {
                if (remaining_possible_added_values.length < 1)
                    return;
                const { bestMatch, bestMatchIndex } = (0, string_similarity_1.findBestMatch)(removed_value, remaining_possible_added_values);
...
const SentencesArray = (paragraphs) => {
    return (0, flattenArray_1.default)(paragraphs.map((paragraph) => {
