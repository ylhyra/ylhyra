...
export function endsInVowel(input: Word | string | undefined) {
  let string;
...
export function endsInConsonant(string: string | undefined) {
...
export function splitOnAll(string: string | undefined) {
...
export function isVowellikeCluster(string: string) {
  // if (typeof string !== 'string') throw new Error('splitOnAll expected string');
  return new RegExp(`(${vowellikeClusters}|[${vowels}]+)`, "i").test(string);
}

/** @hasTests */
export function removeLastVowelCluster(string: string | undefined) {
